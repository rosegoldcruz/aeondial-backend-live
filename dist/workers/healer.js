    ...
    if (staleCalls && staleCalls.length > 0) {
        ...
        for (const call of staleCalls) {
            ...
            if (call.agent_id) {
                ...
                if (!updateCount)
                    console.log(`[HEALER] Agent ${call.agent_id} already released`);
            }
        ...
        if (staleBridgedCalls && staleBridgedCalls.length > 0) {
            ...
            for (const call of staleBridgedCalls) {
                ...
                if (call.agent_leg_id) {
                    ...
                    if (!res)
                        console.log(`[HEALER] Hangup attempt on agent leg ${call.agent_leg_id} (may be already dead)`);
                }
                if (call.lead_leg_id) {
                    ...
                    if (!res)
                        console.log(`[HEALER] Hangup attempt on lead leg ${call.lead_leg_id} (may be already dead)`);
                }
                // Mark call as failed
                await supabase
                    .from('calls')
...
async function run() {
    ...
    setInterval(async () => {
        try {
            ...
            console.error('[HEALER] Bridged cleanup error:', e);
        }
