...
function getTCPATimezone(phone) {
...
function isTCPACallable(phone, timezone) {
...
const dialWorker = new Worker(DIAL_QUEUE, async (job) => {
...
export async function setAgentCooldown(agentId, ms = POST_RELEASE_COOLDOWN_MS) {
    await redis.set(`dialer:agent:${agentId}:locked`, '1', 'PX', ms);
}
async function rollback(agentId, leadIds) {
    await supabase
        .from('agent_sessions')
        .update({ state: 'READY', updated_at: new Date().toISOString() })
        .eq('agent_id', agentId);
...
async function failReservedLeads(agentId, leadIds) {
    await supabase
        .from('agent_sessions')
        .update({ state: 'READY', active_call_id: null, updated_at: new Date().toISOString() })
        .eq('agent_id', agentId);
