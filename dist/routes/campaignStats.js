...
export async function campaignStatsRoute(app) {
    app.get('/stats', { onRequest: [app.authenticate] }, async (_req, reply) => {
        ...
        if (campaignError)
            return reply.status(500).send({ error: campaignError.message });
        ...
        for (const c of (calls ?? [])) {
            if (!c.campaign_id)
                continue;
            if (!map[c.campaign_id])
                map[c.campaign_id] = { attempts: 0, connected: 0, wraps: 0 };
            map[c.campaign_id].attempts++;
            if (c.answered_at)
                map[c.campaign_id].connected++;
            if (!c.disposition)
                map[c.campaign_id].wraps++;
        }
