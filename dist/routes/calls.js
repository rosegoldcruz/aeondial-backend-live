...
async function telnyxAction(callControlId, action, body = {}) {
    ...
    if (!res.ok) {
        ...
        throw new Error(err?.errors?.[0]?.detail ?? `Telnyx ${action} failed`);
    }
    return res;
}
...
export async function callRoutes(app) {
    ...
    app.post('/:id/wrapup', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        if (!wrapupCount)
            console.warn(`[calls] Agent ${agentId} wrapup transition rejected`);
        // Update lead status
        ...
        const { count: readyCount } = await supabase
            ...
            .eq('state', 'WRAP_UP');
        if (!readyCount)
            console.warn(`[calls] Agent ${agentId} READY transition rejected`);
    ...
    app.post('/:id/hangup', { onRequest: [app.authenticate] }, async (req, reply) => {
        const { agentId } = req.user;
        const { id: callId } = req.params;
        const { data: call } = await supabase
            .from('calls')
        ...
        const hangups = [];
        if (call.lead_leg_id)
            hangups.push(hangupCall(call.lead_leg_id).catch(() => null));
    ...
    app.post('/:id/voicemail-drop', { onRequest: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
    ...
    app.post('/manual-dial', { onRequest: [app.authenticate] }, async (req, reply) => {
        const { agentId } = req.user;
        const { phone: rawPhone, lead_id: existingLeadId } = req.body;
        ...
        if (!leadId)
            return reply.status(500).send({ error: 'Could not resolve lead' });
