...
export async function agentRoutes(app: FastifyInstance) {
  app.get('/', { onRequest: [app.authenticate] } as any, async (_req, reply) => {
    ...
    const agentMap: Record<string, { calls: number; totalSecs: number }> = {};
    for (const c of (callCounts ?? [])) {
      if (!agentMap[c.agent_id]) agentMap[c.agent_id] = { calls: 0, totalSecs: 0 };
  ...
  app.get('/stats', { onRequest: [app.authenticate] } as any, async (_req, reply) => {
    ...
    if (agentError) return reply.status(500).send({ error: agentError.message });
    ...
    const map: Record<string, { calls: number; demos: number; secs: number }> = {};
    for (const c of (calls ?? [])) {
      if (!map[c.agent_id]) map[c.agent_id] = { calls: 0, demos: 0, secs: 0 };
      map[c.agent_id].calls++;
      map[c.agent_id].secs += c.duration_seconds ?? 0;
      if (c.disposition === 'Interested') map[c.agent_id].demos++;
    }
